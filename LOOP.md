...
- **卡住跳过**:同一 DoD 项连续 3 轮不闭环、或撞上需人拍板的决策 → 标成 blocker 写进 PROGRESS,去做当前 Phase 内下一个能做的项。不停机空等,不在死磕点烧 token。
- **Phase 被卡死(终止)**:当前 Phase 剩下的未勾项全被 blocker 卡住、无项可推进 → 停下,把 blocker 清单留在 PROGRESS 顶部交给人。不跳到后续 Phase 绕过被卡的前置(尤其 Phase 1 是所有部署的前置,卡死不得跳过)。
