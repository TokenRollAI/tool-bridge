---
description: Linux 设备 daemon(tb daemon)的安装、更新、日常管理、验证边界,以及设备端 SDK 运行时边界注意事项。
kind: guide
code:
  paths:
...
daemon 是 Linux 主机的设备入口；React Native/Hermes 等宿主的设备连接改走 `@tool-bridge/sdk/device` 独立 neutral 子入口，宿主注入 WebSocket、凭证、生命周期与 executor。相关的稳定教训：

- 多运行时 npm 包的兼容性按目标子入口的 JS/d.ts 产物依赖闭包判定，不按包级 `engines`/依赖清单猜测；`ws` 可以继续被安装，但不得进入 device 子入口的模块图。
- 运行时中立要求 JS 与声明文件两条依赖图同时隔离，声明文件也可能泄漏 Node-only 类型。
- 幂等缓存只有在调用标识（call id）能端到端稳定重用时才具有跨重启语义；当前只有进程内同 call id 去重，不要把它承诺成 exactly-once。
