---
description: 设备 WebSocket 会话的持久性与在线状态:Cloudflare Durable Object 休眠、alarm 心跳巡检、presence 三态降级、Node 宿主 sweepOrphans,以及真实 eviction 验证方式。
kind: guide
code:
  paths:
    ...
    - packages/server/src/redisDeviceRouter.ts
    - scripts/verify-device.ts
---

# 设备 WebSocket 会话与 DO 休眠
...
- registry（D1 state store）中的 online 位不是租约；真正发送以 DO 内活动 socket 为准。但发现面不再只凭这个位:
...
- DO 用 auto-response 应答 ping/pong **不唤醒对象**,故心跳无法在收到 pong 时回写 state store。改由周期 alarm
...
- 投影是只读降级:读路径按 `lastSeenAt` 把 online 显示为 stale,但**绝不回写** `setOnline(false)`
  (权威状态只由连接生命周期写)。
- generation guard 防止旧连接污染新连接:close/error、替换连接和过期状态都必须清理,且不得让旧连接覆盖新连接。
- Node 宿主启动 `sweepOrphans` 的崩溃态分支(meta 无 disconnectedAt 却无活连接)也要同步把 registry online
  翻 false,不能只排 reclaim。多副本下"本副本没有该连接"不等于"设备离线",回收前必须查全局路由,查不通时保守跳过。
...
若失败，分别检查设备端 heartbeat、DO storage 的连接归属、D1 state store 中的 presence/路由状态和调用端超时，不要用增加重试掩盖状态恢复错误。
