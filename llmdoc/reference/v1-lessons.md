...
## 文件级检索地图(2026-07-06 已检索,依据 `.llmdoc-tmp/investigations/v1-reference-map.md`)

核心资产集中在 `src/worker/tb/`;主入口 `src/worker/index.ts`(手写路由 + 认证 + 内联 MCP client,1145 行)。

| 子系统 | v1 文件 | 机制一句话 |
|---|---|---|
| MCP 会话管理 | `src/worker/tb/mcp-client.ts`(权威版;`index.ts` L415-650 为等价内联旧版) | **完全无状态、每次调用重建会话**:initialize→拿 `MCP-Session-Id`→initialized(202)→list/call→finally DELETE;响应按 Content-Type 分流 JSON/SSE(手写逐行解析);有界读取(MAX_JSON/SSE_BYTES);全部失败在 `executeMcpRequest` 单点裹成 `UpstreamError→502`。**无会话复用、无 404 重建重试**(re 要新增,Reference §3)。 |
...
7. v1 不含 hono/@modelcontextprotocol/sdk/zod——这三者是 re 的新增基础设施,勿从 v1 抄手写实现(违反 LOOP 纪律 4)。
