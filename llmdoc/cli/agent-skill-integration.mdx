...
feedback 是调用生命周期的实时控制信号,不是事后复盘:前查、异常即查、验证后投票、去重提交都发生在调用现场。运行时 `~help` 是契约真源,feedback 是经验层——可解释坑和规避方式,但不能覆盖当前 schema,也不能把未经验证的猜测包装成结论。
...
在 Skill 仓库至少验证 `npx --yes skills add . --list`,输出必须只发现预期的 `tool-bridge` skill。再做两个隔离前向测试:
