...
pnpm --filter @tool-bridge/server test         # 5 文件 23 例,纯 Node vitest(不需要 workerd)
```
...
|---|---|---|
| StateStore 一致性 | KV 最终一致,吊销窗口上限 60s | SQLite 强一致,吊销即时 |
...
设备协议行为不分叉:hello 验证+落库统一走 gateway `src/deviceHello.ts`(`processDeviceHello`),DO 与 DeviceHub 只是宿主胶水。
