...
- **集成测试默认跑最中立的宿主**:新增或迁移集成用例先自问「它碰了什么只有那个运行时才有的东西」——只有真正触达宿主专属原语(DO/`runInDurableObject`、真实 KV·R2 binding、Static Assets、靠 env binding 开关的 opt-in)的才付重型运行时的启动开销,其余走 `packages/app` 的 Node 套件直打 `createTbApp`(`app.request(url, init)` 与 `SELF.fetch` 签名一致,迁移是机械替换)。**"跑在真实运行时里"不等于验证了真实运行时**;`packages/app/test/` 同在 `types: []` 约束内,测试里用 `process`/`Buffer` 写断言等于绕过被验证的中立面。可选注入点(SearchIndex 等)至少要有一个不依赖外部服务的内存实现,共享逻辑下沉 core、适配器只管存储/查询原语。
- **包边界一变就重算发布闸门**:某个包的产物被别的包 bundle 时,它的测试是那几个包**共同**的 `publish-*.yml` 闸门;这个映射无工具兜底,拆包/并包后逐个 workflow 手动过一遍。
- **代理/存储边界要测出站形状**:白名单/环检测之外,必须有测试直接断言出站请求或存储层的真实形状(调用者 SK 不外传、skRef 换发、路径改写、版本/内容),不能只断言入口返回 200。
- **多服务开发栈不能用 health 冒充链路**:正例须从公开 gateway入口穿过真实中间层并断言最末 mock产生的可辨识结果;再用全新认证缓存键注入错误凭据观察上游拒绝与网关非零失败,恢复后重跑,最后重启有状态服务再跑以证明卷持久。后台 `up -d` 只表示调度成功,one-shot验收用 profile + `run --rm`/等价同步命令传播退出码。固定开发凭据存在时同时 inspect published `HostIp`,确认仅 loopback;teardown分别验证保留卷与删卷语义。
