---
description: Cloudflare 宿主的初始化、部署与验收:tb init cloudflare、gateway 双入口与三条发布路径、部署形态总表、必需安全配置与账户中立约束。
kind: guide
relations:
  requires:
...
Cloudflare 是一种宿主，不是业务真源。仓库中的 `packages/gateway/wrangler.jsonc` 必须保持账户中立：不提交 account ID、域名、D1/R2 ID 或环境凭据；真实账户与资源 ID 由 provision 回填。

## 部署形态总表

| 形态 | 入口 | 适配器 |
|---|---|---|
| Cloudflare Deploy Button 模板 | `template/src/index.ts` | D1(TB_STATE/TB_SEARCH)、R2、Durable Objects、Static Assets |
| Cloudflare Workers(源码) | `packages/gateway/src/deployEntry.ts` | D1(TB_STATE/TB_SEARCH)、R2、Durable Objects、Static Assets |
| Node / Docker | `packages/server` | SQLite/PostgreSQL、文件或 S3 对象存储、ws、静态 Dashboard |
...
KV 已从 Cloudflare 宿主全面撤出，权威状态在 D1（见 [状态存储决策](state-store-decision.mdx)）。Deploy Button template 消费 `@tool-bridge/gateway/full`，与源码部署是同一个产品、同一套装配（含 D1 search）。Node 宿主细节见 [Node、Docker 与 Helm](node-docker-and-helm.mdx)。
...
- 首次引导必须预置 `TB_BOOTSTRAP_ADMIN_SK`，并保存在密码管理器；缺失时 fail closed，不得从日志回收最高权限凭据。
...
真实云资源和上游会产生费用或副作用；每轮最多执行一次，需获得用户授权并把证据留在 PR/CI，而不是写入 llmdoc。
