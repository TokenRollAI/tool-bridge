...
| CLI | 纯 API 客户端 `tb`,23 个顶层命令族一一映射接口面;`tb search` 直连 root `/~search`,**无专用端点** | — | `packages/cli`(commander;npm 包 + Bun 独立二进制);根 `Dockerfile.cli` 是同一完整命令面的 Linux amd64/arm64 非 root 镜像 |
...
| 部署 | CF 与 Docker 两条网关生产路径产出同一棵树;Compose 只编排 localhost 开发验收栈;CLI 镜像是客户端/sidecar 分发物,不是第三种网关宿主 | — | CF:`scripts/provision.mjs` + wrangler;Docker/Node:`packages/server` + 根 production Dockerfile;完整 CLI:`Dockerfile.cli`;开发:`docker-compose.yml`(gateway final image + plugin-feishu Wrangler + authenticated mock upstream + profile smoke),见 [../guides/docker-host.md](../guides/docker-host.md) |
