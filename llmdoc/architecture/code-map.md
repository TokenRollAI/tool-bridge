...
| `search/` | 全局工具搜索共享 contract | `types.ts`(`ToolSearchOptions{mode,limit,cursor}`、轻量 candidate→4 MiB hydrate、只读 `SearchIndex` + `MutableSearchIndex.replace/remove/removePrefix/rebuild`;default50/max200、batch100/work400、500-node audit/20 KiB node snapshot、AES-GCM cursor 绑定 query/mode/revision/offset;长词 FTS+短词 LIKE 的 hybrid AND,raw ToolSpec snapshot/digest/JSON1 budgets);`test/search/searchIndex.fixture.ts` 为 D1/SQLite v2 共用 contract |
...
| `tbApp.ts` | **宿主中立 `createTbApp(deps)`**:全局安全头、认证中间件、`~help`/`~tree`/`~skill`/`~describe`/`~register`/`~feedback`/`~mcp` 与 root-only `POST /~search`;Search 请求 query+mode/limit/cursor,每次 canonical audit 后 over-fetch,按 read+call、kind/config、virtualize 后处理,空不可见页不发 cursor;registry/plugin/fresh tool list/feedback mutation 接 SearchSynchronizer;另含数据面、remote、两级 `~help`、`/ui`、`/~ref` |
...
| `test/` / `scripts/` | workerd 集成测试族;`search.integration.test.ts` 钉 limit/cursor、权限后 over-fetch、4 MiB hydrate 与空不可见页无 cursor;`searchSynchronizer.test.ts` 钉 canonical 独立、dirty audit、hot/full LKG/恢复;`d1SearchIndex.integration.test.ts` 复用 v2 contract并覆盖自动同步、feedback、125 tools、500-path并发 trigger与 48-query预算;`ui.integration.test.ts` 当前 15 cases,钉 `/ui`/node/search deep link、当前构建 chunk 内的 `/~search`、root POST 不被 assets 吞掉及特殊字符 TreePath helper。`scripts/echo-mcp.ts` 兼作 Compose opt-in authenticated TAT/MCP mock(默认 MCP E2E 行为不变),`compose-smoke.ts` 走 gateway → plugin-feishu → mock 的注册/挂载/调用;另有 s3-mock / stub-provider |
...
| `sqliteSearchIndex.ts` | Node keyword v2 adapter:与 StateStore 共用 `state.sqlite3` 文件但用独立连接/独立表;raw ToolSpec/feedback FTS/meta/snapshot schema,事务化 replace/rebuild + 500-path trigger,candidate/hydrate/AES-GCM cursor,复用 shared contract |
...
| `test/` | `bootstrap.test.ts` 钉 Node缺 Admin SK fail closed/显式逃生阀/预置密钥及 canonical origin;`device.integration.test.ts` 用可控 StateStore barrier 固定旧连接→identify await→同 ID replacement窗口,保持旧 session 可调用并断言零下发,且 mutation删除 generation guard时真红;`sqliteSearchIndex.test.ts` 复用 v2 shared contract;`server.integration.test.ts` 覆盖真实 HTTP root search与重启;server 全包当前 38 passed |
...
- `scripts/`:gen-dev-vars.mjs(.env→.dev.vars)、provision.mjs(幂等建 KV/R2/D1;D1 用 `list --json` 精确 name/uuid,existing 同步 id+skip,create 后 re-list,regex 回填 `TB_SEARCH.database_id`;缺 uuid fail closed)、provision.test.mjs(fake wrangler 连跑两次,根 `test:provision` 验证不重建与 id 回填)、smoke.ts(显式 base URL + SK;匿名 401和认证 markdown/DSL 200都执行,缺凭据网络前 fail closed)、verify-device.ts(真实设备 shell/fs、合法 registerPaths及越界 `tb connect` 非零 + 同 ID replacement,用旧 `echo`/新 `printf` 区分代际,可选 hibernation)、verify-revocation.ts / verify-plugin.ts(通用 plugin/v2 验收)、verify-mcp.ts(官方 SDK 的 admin/narrow 双连接;admin 与窄侧都必须真实 call,窄工具默认 `system/status:get`,然后 stale admin-only 名拒绝;不签发 SK/不改生产状态)。
