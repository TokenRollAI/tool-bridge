...
- `index.ts` 注册 17 个命令;`commands/` 每命令一文件(status/login/whoami/use/sk/secret/ls/tree/help/call/tool/server/ctx/connect/device/mount/plugin)。
- 横切:`config.ts`(XDG 配置、多 profile)、`http.ts`(API 客户端)、`output.ts`(`--json`)、`args.ts`、`scope.ts`、`registry.ts`(节点管理助手,rm 前 kind 校验)、`deviceRuntime.ts`(`tb connect` 长驻:partysocket 重连 + 30s 心跳判死链)、`deviceId.ts`。
