...
| `wrangler.jsonc` | 绑定 TB_KV / TB_R2 / TB_DEVICE(DO)/ ASSETS(dashboard dist,`run_worker_first`)及 `TB_SEARCH` → `tb-search` D1(当前真实 UUID `f788b779-ec1c-4fba-ac1f-b780fab990fc`,由 provision 幂等回填)+ `account_id` + custom domain；源码保持账户中立，provision 有 domain 时 `workers_dev:false`+custom route，无 domain 时 `workers_dev:true`+清 routes；Preview URLs 始终禁用,`TB_CANONICAL_ORIGIN` 可固定 OAuth callback origin |
...
- `index.ts` 薄入口(调用 `runMain`);`main.ts`(生产解析入口:递归 `exitOverride`,Commander help/version/解析错误纳入人类/`--json` 统一输出与退出码;JSON 模式按解析后的 option value/source 判断,不扫描裸 argv);`program.ts`(`buildProgram()` 装配命令族、根全局参数与 preAction 合并,递归开启组级 `showGlobalOptions`,`.helpCommand(false)` 保留业务 `tb help [path]`);`commands/` 每命令一文件、导出工厂函数 `xCommand(): Command`(init/status/login/whoami/use/sk/secret/federation/note/feedback/ls/tree/search/help/call/tool/server/ctx/skill/connect/device/mount/plugin/integration);`commands/search.ts` 直连 root `/~search`,构造 `{query,opts:{mode?,limit?,cursor?}}`,JSON 保留 Page、人类输出工具表与 next cursor;`--no-shell` 用 commander 原生否定(`opts.shell === false`)。`skill`(skillhub 命令族)镜像 `ctx`:mount/unmount 走 `~register`/`system/registry`,数据面 ls/get/search/publish/rm 走 `{tool,arguments}` 信封;`publish <dir>` 递归读本地文本文件、`get --out <dir>` 逐文件落盘(遇 `$ref` 提示)。
- `cloudflareInit.ts` / `commands/init.ts` — 唯一非 API 编排例外：从源码 checkout 调 pinned Wrangler，whoami/account → `secret list` 判新旧 → 既有 profile 先认证 → provision/build/deploy/verify/profile。新 trust root 只写 mode 0600 临时 secrets-file；既有 Worker 的 bootstrap Admin SK 从不生成或覆盖。`@clack/prompts` 只承载交互，`p-retry` 只用于新 Worker 部署后的有界就绪探针。
...
- `commands/integration.ts` — **集成的用户面**(`catalog`/`add`/`auth`/`ls`/`rm`)。`add` 是编排非新协议(secret inventory → set → `~register` → oauth 提示),凭证四源互斥(`--key`/`--key-stdin`/`--field k=v`/`--credential`;旧 `--secret` 隐藏兼容),内部槽位用 `integration-v2-${encodeURIComponent(path)}` 无碰撞派生且不进文本/JSON。挂载失败只在分页确认槽位原本不存在时清理,不误删轮换/同名既有凭证;五种输入错本地拒,catalog 查不到/无权限时降级为不给提示而非阻断。`auth` 直接返回 `toolAuthCommand()` **本体**;`ls` 是 registry 投影且把引用收成 credential=`managed|none`;`rm` 的旧 `--purge` 只作隐藏兼容。
...
- `pages/system/forms/`:四套系统表单的独立所有权边界。`managedCredential.ts` 是内置集成凭证的共用编译层(用户态 inline/existing/none → 内部 SecretStore 写入 + authRef,新槽用 `integration-v2-${encodeURIComponent(path)}` 无碰撞派生),`ManagedCredentialFields.tsx` 是「添加集成」与高级「挂载节点」共用的凭证 UI;二者都不向内置集成用户展示 authRef,同 provider/export 替换留空时保留既有/legacy 绑定。`registryConfig.ts` 是六 kind + virtualize 的纯 wire builder,并只对 external/custom plugin 保留原始 authRef 输入;`RegistryKindFields.tsx`/`MountDialog.tsx` 分别负责 kind 字段与 secret set→registry write→失败清理生命周期。`integrationPlan.ts` 复用上述凭证编译层生成集成调用,`IntegrationDialog.tsx` 是 catalog 驱动的向导壳(挂在 RegistryPage 主按钮与 `BuiltinCatalog.tsx` 两处)。`pluginManifest.ts` 是 plugin/v2 纯 builder,`PluginManifestFields.tsx`/`PluginFormDialogs.tsx` 负责字段与注册/编辑 dialog;`skConfig.ts` 是 SK wire builder,`SkFormFields.tsx` 负责 scope/registerPaths 字段。共用 section 在 `components/FormSection.tsx`。
...
- `vitest.config.ts` 使用 Node environment;Dashboard 共 4 个测试文件 / 66 cases:`test/systemForms.test.ts` 与 `test/integrationPlan.test.ts` 断言 Registry/Plugin/SK builder、凭证编译、无碰撞槽位和 fail-closed 分支,`test/integrationDialog.dom.test.tsx` 与 `test/mountDialog.dom.test.tsx` 用轻量 DOM 钉住两条内置集成生命周期(含不暴露 authRef、secret 先写后挂载、失败不误删既有同名凭证)。`test` script 已加入根 `test:unit`;协议/静态接线另由 gateway fresh-source `ui.integration.test.ts` 覆盖。产品级可重跑浏览器回归仍缺;Round 25 真实浏览器核对 desktop/mobile 无横向溢出且 console 0。验收矩阵见 [../guides/verification-and-commit-practices.md](../guides/verification-and-commit-practices.md)。
