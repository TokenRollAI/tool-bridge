...
- `.github/workflows/`:`release.yml`(**一轮发布的编排器**,`workflow_dispatch` + `dry-run`/`packages` 两输入:算顺序 → 打 tag → `gh run watch` 串行等每个 publish 真正结束;拒非 main;tag 已存在则跳过;按 tag 而非 workflow 名找 run,因为 `server-v*` 同时触发 npm 与 GHCR 两个。**只编排不自己发** —— npm Trusted Publishing 的信任按 workflow 文件名授予)+ publish-{cli,sdk,app,gateway,dashboard,plugin-sdk,server}.yml(tag `<pkg>-v*`,npm Trusted Publishing)+ publish-docker.yml(tag `server-v*`,GHCR 镜像,buildx amd64/arm64)+ `ci.yml`(verify + build + bump 提醒)。
