...
| `deviceSession.ts` | `DeviceSession` DO 胶水:WS hibernation、待决表、`setWebSocketAutoResponse`、惰性会话重建;设备子树回收同步 SearchIndex;休眠恢复与每次 invoke 会重验 SK,但当前跨 KV await 后未复核 activeConnId、`markDisconnected` 可用陈旧 meta 覆盖新连接,属待修 TOCTOU(协议行为在 deviceHello.ts) |
...
| `test/` | workerd 集成测试族;`search.integration.test.ts` 钉 limit/cursor、权限后 over-fetch、4 MiB hydrate 与空不可见页无 cursor;`searchSynchronizer.test.ts` 钉 canonical 独立、dirty audit、hot/full LKG/恢复;`d1SearchIndex.integration.test.ts` 复用 v2 contract并覆盖自动同步、feedback、125 tools、500-path并发 trigger与 48-query预算;`ui.integration.test.ts` 钉 `/ui/search` deep link、当前构建 chunk 内的 `/~search`、root POST 不被 assets 吞掉及特殊字符 TreePath helper;`scripts/` 有 echo-mcp / s3-mock / stub-provider |
...
- `pages/`:LoginPage / OverviewPage;`SearchPage.tsx` 是独立 root tool search 页,提交查询后展平 cursor pages,区分权限/404/空态并把结果链接到 NodePage的 `?tool` 预选;`NodePage` 是未知节点的通用协议回退并编排命令、Context、帮助、Note/Feedback tabs。`pages/system/` 的 SkPage / RegistryPage / DevicesPage / SecretsPage / PluginsPage / FederationPage 承载已知 builtin 工作流,cursor 分页与危险操作结算;其中 Registry 覆盖全 NodeConfig/OAuth/virtualize,Plugins 覆盖六动作,SK 覆盖完整 scope,Devices 只表达后端支持的删除语义,Federation 保持 env 条目只读。
...
- 无专用前端测试;协议/静态接线由 gateway 的 `ui.integration.test.ts` 覆盖且该套件启动前从当前 source 重建 Dashboard。产品级可重跑浏览器回归仍缺;Round 24 浏览器证据覆盖桌面/390px、分页/cursor、普通与 `?/#/%` 特殊字符路径的工具预选及 console/横向溢出。验收矩阵见 [../guides/verification-and-commit-practices.md](../guides/verification-and-commit-practices.md)。
...
| `test/` | `sqliteSearchIndex.test.ts` 复用 v2 shared contract(权重/分页/cursor)并覆盖同库异表、500-path cap、重开持久化;`server.integration.test.ts` 覆盖 registry 自动同步后的真实 HTTP root search 与重启;另含 StateStore、device、ui、context 宿主测试 |
