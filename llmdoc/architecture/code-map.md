...
- `scripts/`:gen-dev-vars.mjs(.env→.dev.vars)、provision.mjs(幂等建 KV/R2/D1;D1 用 `list --json` 精确 name/uuid,existing 同步 id+skip,create 后 re-list,regex 回填 `TB_SEARCH.database_id`;缺 uuid fail closed)、provision.test.mjs(fake wrangler 连跑两次,根 `test:provision` 验证不重建与 id 回填)、smoke.ts(显式 base URL + SK;匿名 401和认证 markdown/DSL 200都执行,缺凭据网络前 fail closed)、verify-device.ts(真实设备 shell/fs、合法 registerPaths及越界 `tb connect` 非零 + 同 ID replacement,用旧 `echo`/新 `printf` 区分代际,可选 hibernation)、verify-revocation.ts / verify-plugin.ts(可重跑生产验收;后者是通用 context plugin/v2 + 显式 export 全流程,不代表飞书业务动词)。
