...
- [guides/npm-publish.md](guides/npm-publish.md) — cli / sdk / gateway / dashboard / server 五包的 npm 发布:tsup bundle + dts 内联(paths 映射 + 隔离 tsc 验证)、publishConfig 覆盖模式(pnpm pack + publish tarball)、tag 触发 CI(Trusted Publishing OIDC)、新包"手动首发+配 Trusted Publisher"两段式(dashboard 先于 server)、EOTP/provenance E422 等坑。
