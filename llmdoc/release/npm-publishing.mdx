---
description: npm 版本 bump 判断、发布顺序、打包与 tarball 验证闸门:七个公开包、pack-and-verify 脚本、registry 复验与产物漂移防护。
kind: guide
code:
  paths:
...
3. 执行 `pnpm verify` 和 `pnpm turbo run build`（公开包改动必须补全仓 build，`verify` 不含 build）。
...
7. 用 `npm view <pkg> version` 确认 registry 元数据后，再下载 registry 中的精确版本 tarball，重复干净安装与烟测。

tag 前缀与目录同名，包括 `plugin-sdk-`。Dashboard 若嵌入 server/gateway 产物，应先发布它依赖的包，再发布承载最终产物的包。

## 三种独立证据

registry 元数据存在、tarball 可被 npm 安装、安装后的 bin 可执行,是**三道互不替代的独立闸门**——只满足第一项不代表版本可用（历史上 `catalog:` 协议残留导致 latest 版本对 npm 消费者不可安装）。发布后仍须从 registry tarball 复验，`npm view` 不能代替。
...
- publish workflow 必须捕获脚本返回的 tarball 路径，并将**同一个文件**交给 `npm publish`；校验后重新打包会留下产物漂移窗口。
