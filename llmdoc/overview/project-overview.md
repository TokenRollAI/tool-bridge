...
公共站点解释稳定的产品模型、上手路径和部署方式，不维护某个实例动态生成的完整工具目录。发生差异时，真源优先级是：目标实例的 `~help` / `~tree` / JSON Schema、对应版本的代码与发布说明、公共教程。用户可感知的契约或部署入口变化要同轮评估是否更新公共文档。

站点在自身仓库验证通过后由 `main` 自动发布到 Cloudflare Pages；PR 使用预览部署。Cloudflare account、API token、项目和域名绑定由平台侧配置，不回填到本仓库或公共站点源码。
