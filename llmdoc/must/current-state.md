...
- **npm 已发布**(四包均经 CI Trusted Publishing;core 为 private 不发布):`@tool-bridge/cli` 0.6.1(2026-07-09,markdown 终端富渲染 + help 描述重整;0.6.1 修 GFM 单波浪号删除线吃掉 ~ 保留段——同行 /~help 与 /~tree 被配对,CLI 渲染器禁 del tokenizer,Dashboard remark-gfm 不受影响)、`@tool-bridge/sdk` 0.4.0、`@tool-bridge/gateway` 0.4.0、`@tool-bridge/dashboard` 0.5.0(2026-07-10,响应式功能性重构;tag `dashboard-v0.5.0`,Trusted Publishing run `29046855598` 成功且 npm `latest=0.5.0`)。gateway/dashboard 的 Trusted Publisher 已配置生效;`@tool-bridge/server` 0.1.0 可发布,**待手动首发 + 配 Trusted Publisher**。发布流程见 [../guides/npm-publish.md](../guides/npm-publish.md)。**坑:一次推多个 tag(`git push origin tag1 tag2 …`)不触发 tag workflows,须逐个 push**(2026-07-08 实测:四 tag 同推零触发,删除后逐个重推全部触发;同日逐个推四 tag 复验全部正常触发)。
...
- 2026-07-10:**Dashboard 功能性前端重构已发布并在生产生效**——在保留 graphite + signal amber 工业控制台识别的前提下,落地桌面侧栏/移动抽屉的响应式 AppShell、登录/总览/节点及全部管理页适配;调用历史升级为 profile id + BaseURL 隔离的 metadata allowlist v2,换凭据以 revision 隔离缓存并清理 Query/Mutation cache;builtin 列表统一 cursor 分页,Devices 删除不再承诺后端不支持的递归语义,Context 编辑以正文/version/`$ref` 原子基线提交,危险操作端到端等待 `mutateAsync`;全部路由与 RJSF/AJV 分包并加 `AppErrorBoundary`。本地真实浏览器验收覆盖桌面/移动/矮屏滚动、抽屉 Escape 焦点恢复、首屏仅 `~tree?depth=1`(打开命令面板才取 depth=8)、敏感调用后 localStorage 无参数值、55 条 Secret 跨默认 50 条页大小加载;QA 仅使用临时本地数据,未触碰生产。`pnpm verify` 全仓 **1005 passed / 7 skipped**,退出码 0。发布证据:npm `@tool-bridge/dashboard` 0.5.0(tag `dashboard-v0.5.0`,Actions run `29046855598`);生产 `/ui/` 由 Worker version 43 承载,线上/本地及深链接 HTML SHA-256 均为 `69cb8606f7a783d88bce55fa473ecb488b1f51bdfba5a846772635ac860cda65`,入口 JS/CSS 均 200,生产 smoke 通过。

## 已部署资源(DJJ 账户)

| 资源 | 名称/地址 | 备注 |
|---|---|---|
| Worker | `tb-gateway` @ https://tool-bridge.pdjjq.org | custom domain(zone pdjjq.org);`wrangler.jsonc` 已写死 `account_id`;DO `DeviceSession` 绑定 `TB_DEVICE`(migration v1,sqlite);Dashboard 经 Static Assets 挂 `/ui`(`run_worker_first: true`);当前生产为 Worker version 43(`dc380d92-fde5-43c4-bf55-d054e631e08c`,100% 流量),运行时 `/healthz` 版本属 Gateway 0.4.0,Static Assets 中 Dashboard 为 0.5.0(两者版本归属不可混用) |
...
- `packages/cli` — commander 框架(严格解析:未知 flag/子命令报错,防权限误配),**162 个单测**(含拼错 flag 事故回归 strictParsing),**20 个命令**:status / login / whoami / use / sk / secret / federation / note / feedback / ls / tree / help / call / tool / server / ctx / connect / device / mount / plugin;全局 `--json`;配置 `~/.config/tool-bridge/config.json`(XDG,多 profile)。
...
- `packages/dashboard` — React 19 + Vite + Tailwind 4 + shadcn/ui + @rjsf + TanStack Query SPA(可发布纯静态产物包:只发 dist,依赖全在 devDependencies):响应式 AppShell(桌面侧栏/移动抽屉)+ 登录/总览/节点/管理页;树导航懒加载(首屏 depth=1,展开按需拉,remote 纯透传),⌘K 打开时才取 depth=8;cmd 表单调用(RJSF/AJV 独立 lazy chunk);context 条目浏览(metadata 编辑、`$ref` 大对象经 Update 只改 metadata、Search mode keyword|semantic、正文/version/`$ref` 原子编辑基线);SK·Registry·Devices·Secrets·Plugins·Federation 管理页(cursor 统一分页;Devices 只陈述 Registry 叶节点删除语义;Plugins 对等 `tb plugin` 六 cmd;Registry 挂载面对等 CLI;Federation 对等 `tb federation ls|add|rm`);调用历史 v2 只保留 metadata,按 profile id + BaseURL 隔离,session revision 隔离凭据变更后的查询缓存;路由全量 `React.lazy`,顶层 `AppErrorBoundary` 提供 chunk/runtime 失败恢复。无专用后端(同源消费 HTBP),行为由 gateway 的 `ui.integration.test.ts` 覆盖;`pnpm deploy:all` 先 build dashboard 再部署 gateway。
...
- `pnpm verify` — typecheck + lint + 单测 + 集成测试一把过(2026-07-10 实跑:681 core + 162 cli + 12 sdk + 119 gateway + 23 server + 8 plugin-feishu = **1005 passed**;SDK 1 + gateway 6 opt-in = **7 skipped**;根 `test:integration` 含 gateway + server + plugin-feishu)。
...
- **端到端验收系统化**:七个 User Case 的脚本化 E2E(CLI `--json` 驱动 + Dashboard 浏览器侧)未拉通;现有 smoke + 三个 verify-* 脚本覆盖主链路。Dashboard 本轮只有高价值手工 Playwright 验收,产品级可重跑浏览器回归仍缺(至少应固化敏感历史、首屏 depth、移动抽屉焦点与 cursor 第二页)。
- **遗留小项**:CLI deviceRuntime 迁移为 SDK 消费者;R2 Access Key 创建后 presign 主路径 opt-in 复验;生产 `device/demo-mac` 疑似测试残骸待确认清理;dashboard 本地开发流程(vite dev + wrangler dev 联调)下次涉及时补 guide。
