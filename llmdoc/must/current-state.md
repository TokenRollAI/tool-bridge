...
- **npm 已发布**(四包均经 CI Trusted Publishing;core 为 private 不发布):`@tool-bridge/cli` 0.6.1(2026-07-09,markdown 终端富渲染 + help 描述重整;0.6.1 修 GFM 单波浪号删除线吃掉 ~ 保留段——同行 /~help 与 /~tree 被配对,CLI 渲染器禁 del tokenizer,Dashboard remark-gfm 不受影响)、`@tool-bridge/sdk` 0.4.0、`@tool-bridge/gateway` 0.4.0、`@tool-bridge/dashboard` 0.4.0(2026-07-08,内容协商默认翻转 markdown;此前 0.4.0/0.3.0 批次为 mcp 托管 OAuth + 联邦白名单 + 树懒加载 + mcp 自定义请求头)。gateway/dashboard 的 Trusted Publisher 已配置生效;`@tool-bridge/server` 0.1.0 可发布,**待手动首发 + 配 Trusted Publisher**。发布流程见 [../guides/npm-publish.md](../guides/npm-publish.md)。**坑:一次推多个 tag(`git push origin tag1 tag2 …`)不触发 tag workflows,须逐个 push**(2026-07-08 实测:四 tag 同推零触发,删除后逐个重推全部触发;同日逐个推四 tag 复验全部正常触发)。
...
- 2026-07-10:**Dashboard 功能性前端重构完成,尚未部署**——在保留 graphite + signal amber 工业控制台识别的前提下,落地桌面侧栏/移动抽屉的响应式 AppShell、登录/总览/节点及全部管理页适配;调用历史升级为 profile id + BaseURL 隔离的 metadata allowlist v2,换凭据以 revision 隔离缓存并清理 Query/Mutation cache;builtin 列表统一 cursor 分页,Devices 删除不再承诺后端不支持的递归语义,Context 编辑以正文/version/`$ref` 原子基线提交,危险操作端到端等待 `mutateAsync`;全部路由与 RJSF/AJV 分包并加 `AppErrorBoundary`。本地真实浏览器验收覆盖桌面/移动/矮屏滚动、抽屉 Escape 焦点恢复、首屏仅 `~tree?depth=1`(打开命令面板才取 depth=8)、敏感调用后 localStorage 无参数值、55 条 Secret 跨默认 50 条页大小加载;QA 仅使用临时本地数据,未触碰生产。`pnpm verify` 全仓 **1005 passed / 7 skipped**,退出码 0。
