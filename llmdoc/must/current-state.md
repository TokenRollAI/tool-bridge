...
- **npm 已发布**(四包均经 CI Trusted Publishing;core 为 private 不发布):`@tool-bridge/cli` 0.6.1(2026-07-09,markdown 终端富渲染 + help 描述重整;0.6.1 修 GFM 单波浪号删除线吃掉 ~ 保留段——同行 /~help 与 /~tree 被配对,CLI 渲染器禁 del tokenizer,Dashboard remark-gfm 不受影响)、`@tool-bridge/sdk` 0.4.0、`@tool-bridge/gateway` 0.4.0、`@tool-bridge/dashboard` 0.6.0(2026-07-10,tree-first 整站大修;tag `dashboard-v0.6.0`,Trusted Publishing run `29068385868` 成功且 npm `latest=0.6.0`)。gateway/dashboard 的 Trusted Publisher 已配置生效;`@tool-bridge/server` 0.1.0 可发布,**待手动首发 + 配 Trusted Publisher**。发布流程见 [../guides/npm-publish.md](../guides/npm-publish.md)。**坑:一次推多个 tag(`git push origin tag1 tag2 …`)不触发 tag workflows,须逐个 push**(2026-07-08 实测:四 tag 同推零触发,删除后逐个重推全部触发;同日逐个推四 tag 复验全部正常触发)。
