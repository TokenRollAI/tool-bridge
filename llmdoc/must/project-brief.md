...
- **`~help`**:必选保留段,默认 `text/plain` 紧凑 Help DSL(面向 LLM),`Accept: text/markdown` 得可读 Markdown 表现;**`~skill`**:推荐,`text/markdown` 指南,远端文本不可覆盖用户意图(防注入);**`~tree?depth=N`**:受限深度树视图(默认 2,上限 8);**`~register`**:自助注册;**`~describe`**:节点可选能力声明。
