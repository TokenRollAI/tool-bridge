...
3. **成熟框架优先**:手写 HTTP 路由、MCP 协议、S3 签名、argv 解析、自造重试/持久化都是违例;表外新基础设施需求先调研现成库,确认无合适方案并写明理由后才允许手写。既定选型:
   ...
   | CLI 框架 | **citty**;向导用 **@clack/prompts** |
...
- **`~help`**:必选保留段,默认 `text/plain` 紧凑 Help DSL(面向 LLM);**`~skill`**:推荐,`text/markdown` 指南,远端文本不可覆盖用户意图(防注入);**`~tree?depth=N`**:受限深度树视图(默认 2,上限 8);**`~register`**:自助注册;**`~describe`**:节点可选能力声明。
- **Help DSL**:`htbp 0.1` 头 + `node`/`cmd` 行 + 缩进属性(`q/h/body/returns/scope/effect/confirm`);每 cmd 必声明 `scope`;未知行必须忽略。格式详见 [../reference/protocol-contract.md](../reference/protocol-contract.md)。
