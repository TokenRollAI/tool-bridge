...
interface Node {
  path: TreePath                        // 唯一键
  kind: 'directory' | 'mcp' | 'http' | 'tool' | 'builtin' | 'context' | 'device' | 'remote'
...
type NodeConfig =
  ...
  | { kind: 'tool',    provider: string }                      // tool-provider plugin id,或 SDK 进程内
                                                               //   保留 id "@local"(Phase 5 定型)
...
**tool 节点(规范性,Phase 5 定型回写)**:`kind:'tool'` 是 §8.1"可被 `NodeRegistry.Write` 以 `provider:<plugin-id>` 引用挂载"对 tool-provider 的合法落点——此前 NodeConfig 仅 context 有 provider 字段,tool-provider Plugin 与 SDK 本地工具源(§7 `registerTool`,provider 用进程内保留 id `@local`)均无合法 kind。cmd 名 = 工具原名(与 mcp/http 一致,§1.3)。
...
- **file**:本地目录(仅 node 宿主:Docker 部署与设备 fs 复用);`List` = readdir,`Search(keyword)` = 名称/内容 grep,路径穿越必须拒绝(规范性:解析后必须仍在根内;含 symlink——须做 realpath-in-root 校验,符号链接解析后逃逸根目录同样拒绝)。
...
**deviceId 缺省生成(实现注记,Phase 4 定型)**:设备侧未显式指定时,deviceId = `os.hostname()` 规范化(小写、非法路径段字符替换为 `-`);首次生成后持久化进 CLI config(`~/.config/tool-bridge/config.json`)固定,`--device-id` 显式覆盖。断线重连恢复 `online` 依赖同 deviceId(§6.3),故 deviceId 必须跨进程稳定。SDK `connect`(§7)缺省同为 hostname 规范化但**不持久化**——嵌入方要稳定重连应显式传入 `deviceId`;CLI 的 config 持久化行为不变(Phase 5 定型回写:SDK 场景无 CLI config 可写)。
...
```ts
// 设备 → 网关(连接后第一帧)
{ type: 'hello', deviceId: string, mountPath?: TreePath, expose: DeviceExpose }
//   mountPath 缺省 device/<deviceId>;registerPaths 受限的 SK 经此声明非默认挂载前缀(§6.1、§2.4a)
interface DeviceExpose {
  shell?: { description?: string, allow?: string[] } // 挂 <mountPath>/shell(工具节点;allow 白名单语义见下)
  fs?:    { roots: string[], readOnly?: boolean }  // 挂 <mountPath>/fs(context 节点,file provider;多根语义见 §6.3)
...
// 网关 → 设备:拒绝帧(发送后网关 close(1008))
//   设备侧以此区分「权限拒绝(不重连)」与「可重试断线(重连)」
{ type: 'error', error: TBError }
...
**shell 命令白名单(规范性,Phase 4 定型)**:`allow` 缺省 `[]` = 拒绝一切命令(默认拒);单值 `['*']` = 放行全部;其余条目对 `command` 做 shell-word 切分取 argv[0] 的 basename,与条目**精确匹配**(如 `allow: ["echo","git"]` 匹配 `echo hi` 与 `/bin/echo hi`);白名单非 `['*']` 时,`command` 含 shell 元字符(`;`、`|`、`&`、`$(`、反引号、`>`、`<`)→ 直接拒。判定在**设备侧执行前**完成,未匹配 → `permission_denied`(403,`retryable: false`);网关侧节点级 `call` scope 判定照旧先行。
...
- `ready` 后,网关代写 NodeRegistry:`<mountPath>`(directory,`online:true`)+ expose 声明的子节点。代写时对每个节点(含 `nodes` 中的自定义节点)逐节点执行 §2.4 判定(含 `conflict` 与保留段校验),`registeredBy` 记设备 SK 身份。

  **`nodes` 自定义节点的调用转发(规范性,Phase 5 定型回写)**:代注册时网关对每个自定义节点写入 providerConfig 标记(`deviceId` + `mountPath`);调用命中设备后代节点 → 经 §6.2 帧协议 `call` 转发(`path` 为相对 `mountPath` 的路径),设备侧按相对路径派发到本实例注册的 Provider;节点 `~help` 来自注册时随 `NodeInput` 提交的工具表(SDK 侧把 ToolDef 随 meta 上送),不新增帧类型——理由:补齐 SDK `registerTool` → `connect` → 远程调用全链路此前缺失的网关侧转发语义。
- 同 deviceId 并发二连:新顶旧——新连接 hello 判定通过后,网关先关闭旧 WS,再向新连接回 `ready`。
- 断线:节点保留、标记 `online:false`,调用返回 §0.2 的设备离线错误;重连(同 deviceId + 同 SK)恢复 `online:true`。
- 超过回收期(默认 24h,部署配置 `TB_DEVICE_RECLAIM_SEC` 可调)未重连 → 自动 `Delete` 该子树。
- `shell` 工具契约:`cmd exec`,args `{ command: string, cwd?: string, timeoutMs?: number }`,返回 `{ stdout, stderr, exitCode }`;`~help` 必须标注 `effect destructive` + `confirm`,且 allow 白名单进 `exec` cmd 的 `h` 行描述(如 `h 允许命令: echo, git;其余拒绝` / `h 允许命令: *`),使 Agent 调用前可预判。
- `fs` context 契约:即 §5 的 file provider 语义,root 为 expose 声明的 roots。多根语义:entry 路径首段 = 各 root 的 basename(多根命名空间);basename 冲突 → hello 拒(`invalid_argument`);单根同样保留 basename 前缀(语义不分叉)。
...
interface PluginRegistry {              // 挂载为 builtin 节点 system/plugin;需 admin
  ...
  Update(id: string, patch: Partial<PluginManifest>): PluginRegistration  // pluginToken 仅当 auth 切至 platform-token(附A 注记)
...
注册时平台自动:探活(`healthPath`)→ 抓 `~help`/`~describe` 做契约校验(方法集合与 `interfaceVersion` 不符则拒绝)→ 可被 `NodeRegistry.Write` 以 `provider: <plugin-id>` 引用挂载(tool-provider 挂 `kind:'tool'`,context-provider 挂 `kind:'context'`,§3.2)。

**契约校验方法(规范性,Phase 5 定型回写)**:校验器带 `Accept: application/json` 请求 `~help`,优先解析 HelpJson 的 `cmds` 名字集合;非 JSON 响应退化为 DSL 行级解析(`cmd ` 行前缀提取)。要求集合:`context-provider/v1` ⊇ {List, Get, Update, Write};`tool-provider/v1` ⊇ {List, Get, Call};可选方法必须与 `~describe.capabilities` 对齐;`kind` 与 `interfaceVersion` 前缀不一致 → `invalid_argument` 拒——理由:§8.2 只承诺 `~help` 是 Help DSL,校验"方法集合相符"需要机器可读来源。

**`platform-token` 语义(规范性,Phase 5 定型回写)**:`auth.kind='platform-token'` 时,`Write` 由平台 mint 一把 SK(owner `plugin:<id>`,初始 scopes 空 = 无权限,按需再授)作为 `pluginToken` 仅在注册响应返回一次;明文同时存入 SecretStore 保留名 `plugin-token:<id>`,平台→Plugin 调用(§8.3)以该 token 作 Bearer(Plugin 借此验证请求真伪,亦可回调平台);`bearer` kind 则从 `secretRef` 解析注入——理由:原文只定义 pluginToken"回调平台用",未定义平台→Plugin 调用的凭证来源。
...
**探活落地(实现注记,Phase 5 定型回写)**:Phase 5 = 注册时探活(失败拒注册)+ `system/plugin` 的 `health` cmd 按需探活(`tb plugin health` 的数据源);周期性 Cron 探活与 Dashboard 告警属 Phase 6+。健康态存独立 key `pluginhealth:<id>` → `{ healthy, checkedAt, consecutiveFailures }`,不污染规范形状的 manifest 记录——理由:Workers 无常驻定时器,且本节语义是"标记 + 告警,不注销",按需刷新不违反。

**plugin-backed context 节点的 `~describe`/`~help`(规范性,Phase 5 定型回写)**:挂载 `provider: <plugin-id>` 的 context 节点,`~describe` 返回 plugin 注册时抓取并缓存的 capabilities(非内置固定表);`~help` 只列四动词 + 已声明的可选方法——理由:未声明的可选方法平台永不调用,亦不应出现在可发现面。
...
**平台重试策略(规范性,Phase 5 定型回写)**:平台仅对 `retryable: true` 的 TBError 与网络失败重试 **1 次**,重试时 `X-TB-Request-Id` 不变;Request-Id 生成 = UUID——理由:本节原只定义"重试时不变"未定次数,与 §4.2 mcp 会话失效重建重试一次的先例对齐。
...
| `tb connect` / `tb mount fs` | §7 `connect` / §6 帧协议(参数注记见表后) |
...
| `tb plugin register/list/get/update/rm/health` | §8.1/§8.2(`rm` = `PluginRegistry.Delete`,附A 原未列,Phase 5 按三入口对等原则补齐——CLI 不得有旁路) |
...
> **`tb connect` 参数注记(Phase 4 定型回写)**:`--device-id`(覆盖缺省 deviceId,见 §6.1 注记)/ `--path <mountPath>` / `--allow <cmd>`(可重复;或 `--allow '*'` 放行全部,白名单语义见 §6.2)/ `--fs <root>`(可重复)/ `--fs-readonly` / `--no-shell`;`tb mount fs <root>` = `tb connect --no-shell --fs <root>` 的语法糖(独立长驻进程)。

> **`tb plugin` 实现注记(Phase 5 定型回写)**:`system/plugin` builtin 的 cmd 表 = `list / get / write / update / delete / health`(全部需 admin,§4.2 小写命名规范);CLI 入口 `tb plugin register --file <manifest.json>`(→ write,回显 PluginRegistration,pluginToken 仅此一次)、`tb plugin update <id> --file <patch.json>`(→ update,patch 为 Partial<PluginManifest>;auth 切到 platform-token 时响应含一次性 pluginToken,回显同 register)——理由:附A 行原缺 Delete/Update 对应命令,补齐后三入口(CLI/Dashboard/HTTP)对 PluginRegistry 能力对等。
>
> **`Update` 语义(Phase 5 定型回写)**:patch 合并后整体重校验;`endpoint`/`healthPath`/`kind`/`interfaceVersion` 任一变更时按注册同流程重探活 + 重抓 `~describe`/`~help` 契约校验并刷新 `pluginmeta:<id>`/`pluginhealth:<id>`(失败拒更新不落库);仅本地字段(`enabled` 等)变更跳过探活——禁用一个已不可达的 plugin 不应被探活挡住。`auth.kind` 切换:platform-token → bearer 吊销旧 SK 与 SecretStore 明文;bearer → platform-token mint 新 SK(pluginToken 仅该响应一次);同 kind 不换发(换发走重注册 write)。
