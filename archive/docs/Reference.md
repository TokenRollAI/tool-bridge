...
| Dashboard UI 组件 | **shadcn/ui + Tailwind CSS**(Radix 底座,管理台的表单/表格/树导航/对话框组件齐备;**不用 antd**——2026-07-06 用户定案;不手写组件库、不用裸 index.html 交差) | <https://ui.shadcn.com> |
| JSON Schema → 表单 | **@rjsf/shadcn(rjsf v6)+ @rjsf/validator-ajv8**:`~help` JSON 表现中 cmd 的 inputSchema 直接喂给它自动渲染表单 | <https://rjsf-team.github.io/react-jsonschema-form/>;**spike 已过**(2026-07-06):@rjsf/shadcn 6.6.2 peer `react>=18`,React 19 + Vite + Tailwind v4 下 string/number/boolean/enum/object/array 全形状渲染正常;须在 CSS 加 `@source '../node_modules/@rjsf/shadcn'` 纳入 Tailwind 扫描 |
