/**
 * 上游错误归一 + https 强制(Proto §4.2 / §3.2 上游 https,Phase 2 定型)。
 *
 * mcp/http Provider 的**单一 choke point**:把上游传输/协议错误归一为 TBError(§0.2)。
...
/**
 * 上游 endpoint https 强制(Proto §3.2 定型):非 https 且未放行 → invalid_argument。
