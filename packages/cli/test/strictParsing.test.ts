...
describe('未知 flag 必须报错(事故回归)', () => {
  ...
  it('每个叶子命令的未知 flag 都报错', async () => {
    const cases: string[][] = [
      ['setup', 'status', '--bogus'],
      ['setup', 'pair', '--directory', '/tmp/bootstrap', '--bogus'],
      ['setup', 'defaults', '--token-file', 'token', '--bogus'],
      ['setup', 'configure', '--token-file', 'token', '--bogus'],
      ['setup', 'recover', '--token-file', 'token', '--bogus'],
      ['config', 'schema', '--bogus'],
      ['config', 'get', '--bogus'],
      ['config', 'status', '--bogus'],
      ['config', 'validate', '--bogus'],
      ['config', 'update', '--revision', '0', '--bogus'],
      ['config', 'apply', '--revision', '0', '--bogus'],
      ['storage', 'list', '--bogus'],
      ['storage', 'get', 'id', '--bogus'],
      ['storage', 'add', '--bogus'],
      ['storage', 'test', 'id', '--revision', '0', '--bogus'],
      ['storage', 'activate', 'id', '--revision', '0', '--active-revision', '0', '--bogus'],
      ['storage', 'rm', 'id', '--revision', '0', '--bogus'],
      ['storage', 'update', 'id', '--revision', '0', '--bogus'],
      ['deployment', 'schema', '--bogus'],
      ['deployment', 'get', '--bogus'],
      ['deployment', 'status', '--bogus'],
      ['deployment', 'update', '--revision', '0', '--bogus'],
      ['deployment', 'agent', '--compose', 'compose.yml', '--bogus'],
      ['maintenance', 'status', '--bogus'],
      ['maintenance', 'database', '--revision', '0', '--instance-id', 'instance', '--bogus'],
      ['maintenance', 'rotate-database-credentials', '--revision', '0', '--instance-id', 'instance', '--bogus'],
      ['maintenance', 'redis', '--revision', '0', '--bogus'],
      ['keys', 'status', '--bogus'],
      ['keys', 'rotate', '--revision', '0', '--target', 'encryption', '--bogus'],
      ['keys', 'retire', 'key', '--revision', '0', '--target', 'encryption', '--bogus'],
      ['keys', 'resume', 'job', '--bogus'],
      ['keys', 'backup', '--out', 'backup.json', '--bogus'],
      ['status', '--bogus'],
      ['login', '--bogus'],
      ['whoami', '--bogus'],
      ['use', 'p', '--bogus'],
      ...
      ['ctx', 'mount', 'p', '--provider', 'storage', '--bogus'],
      ['ctx', 'unmount', 'p', '--bogus'],
      ['store', 'upload', 'photo.jpg', '--bogus'],
      ['store', 'stat', 'store://default/obj_1', '--bogus'],
      ['store', 'get', 'store://default/obj_1', '--bogus'],
      ['store', 'share', 'store://default/obj_1', '--bogus'],
      ['store', 'revoke-share', 'share_1', '--bogus'],
      ['store', 'rm', 'store://default/obj_1', '--bogus'],
      ['store', 'list', '--bogus'],
      ['skill', 'ls', 'hub', '--bogus'],
      ...
      ['device', 'ls', '--bogus'],
      ['device', 'session', 'start', 'p', '--bogus'],
      ['device', 'session', 'list', 'p', '--bogus'],
      ['device', 'session', 'observe', 'p', 's', '--bogus'],
      ['device', 'session', 'stop', 'p', 's', '--bogus'],
...
describe('缺 required option / positional 必须报错', () => {
  it.each([
    ...
    // call 只接受完整命令路径，不存在额外的 required option。
    ...
    [['store', 'upload'], 'commander.missingArgument'],
    [['store', 'stat'], 'commander.missingArgument'],
    [['store', 'get'], 'commander.missingArgument'],
    [['store', 'share'], 'commander.missingArgument'],
    [['store', 'revoke-share'], 'commander.missingArgument'],
    [['store', 'rm'], 'commander.missingArgument'],
