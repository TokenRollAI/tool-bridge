...
const tb = createToolBridge({
  state: new MemoryStateStore(),      // 或显式注入的 PostgreSQL StateStore
  objects: new MemoryObjectStore(),   // 必填；仅示例/测试，生产须注入持久 S3 或自定义 driver
  adminSk: process.env.TB_ADMIN_SK,   // 首次引导必须显式提供；SDK 不自行读取宿主 env
})
...
持久 Store 还需显式注入 `storeRepository` 和 `storeBackends`；Mailbox 需注入 `mailboxRepository` 与加密根。Node 宿主提供 `PgStoreRepository` / `PgMailboxRepository`，初始化表结构后供嵌入应用使用。单独的 StateStore 或字节 driver 不会被当作具备事务保证的持久领域后端。
...
const connection = connectDevice({
  ...
  handler: async (call) => {
    ...
    if (path === 'camera/capture') {
      ...
      // relay PUT 成功即 ready；S3 直传由 SDK 自动 complete。
...
网关 relay 或 S3 presigned PUT，最终只返回稳定 `store://default/...` descriptor。远程 call
...
Node 22+ 的嵌入宿主从 `@tool-bridge/server` 导入官方 AWS SDK 适配器并传给 `objects`。
适配器仅支持受限流式 relay；中立 SDK 不携带 Node/S3 SDK 依赖：

```ts
import { createToolBridge, MemoryStateStore } from '@tool-bridge/sdk'
import { createS3ObjectStore } from '@tool-bridge/server'
...
| `objects`(必填) | default Store 的字节存储，也供对象型 Context 使用；生产必须注入持久 S3 或自定义 driver，`MemoryObjectStore` 只适合测试/易失开发 |
