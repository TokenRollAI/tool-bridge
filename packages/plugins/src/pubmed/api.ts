...
/**
 * 取值时**再解一次**数值字符引用 —— PubMed 的数据里存在双重编码
 * (`&#x26;#x3B1;` 解一次得 `&#x3B1;`,要解两次才是 α)。
 *
 * 只解数值引用、**不解命名实体**:这一步跑在解析之后,命名实体那时已经解过了,
 * 再解一遍会把正文里合法的 `&amp;lt;`(作者真的想写 `&lt;` 这四个字符)吃成 `<`。
 * 上游 `decodeNumericCharacterReferences` 划的就是这条线,照它来。
 */
...
function textOf(element: XmlElement | undefined): string {
  ...
  return decodeNumericEntities(raw).replace(/\s+/gu, ' ').trim()
}
...
/**
 * ID Converter 是 PMC 上另一台主机的接口:它**不吃 `api_key`**,也不需要回查文献正文,
 * 故这是八个 handler 里唯一**不收 `ctx`** 的 —— handler 表的类型允许少收尾部参数,
 * 声明一个用不到的 `ctx` 只会让人误以为这条路径也带凭证出站。
 */
