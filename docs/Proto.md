...
| `tb plugin register/list/rm/health` | §8.1/§8.2(`rm` = `PluginRegistry.Delete`,附A 原未列,Phase 5 按三入口对等原则补齐——CLI 不得有旁路) |
...
> **`tb plugin` 实现注记(Phase 5 定型回写)**:`system/plugin` builtin 的 cmd 表 = `list / get / write / update / delete / health`(全部需 admin,§4.2 小写命名规范);CLI 入口 `tb plugin register --file <manifest.json>`(→ write,回显 PluginRegistration,pluginToken 仅此一次)——理由:附A 行原缺 Delete 对应命令,补齐后三入口(CLI/Dashboard/HTTP)对 PluginRegistry 能力对等。
