...
**deviceId 缺省生成(实现注记,Phase 4 定型)**:设备侧未显式指定时,deviceId = `os.hostname()` 规范化(小写、非法路径段字符替换为 `-`);首次生成后持久化进 CLI config(`~/.config/tool-bridge/config.json`)固定,`--device-id` 显式覆盖。断线重连恢复 `online` 依赖同 deviceId(§6.3),故 deviceId 必须跨进程稳定。
